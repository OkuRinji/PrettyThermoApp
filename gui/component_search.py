...
class ComponentSearchDialog:
    ...
    def _create_widgets(self):
        ...
        ttk.Button(search_frame, text="Найти", command=self._search).pack(side='left', padx=5)
        ...
        ttk.Button(btn_frame, text="✅ Выбрать", command=self._on_select, width=15).pack(side='left', padx=5)
        ttk.Button(btn_frame, text="❌ Отмена", command=self.dialog.destroy, width=15).pack(side='left', padx=5)
        
        # Инфо о выбранном
        info_frame = ttk.LabelFrame(self.dialog, text="Информация о компоненте", padding=10)
        info_frame.pack(fill='x', padx=10, pady=5)
        
        self.info_var = tk.StringVar(value="Выберите компонент из списка")
        ttk.Label(info_frame, textvariable=self.info_var, wraplength=700).pack(anchor='w')
    ...
    def _update_info(self):
        ...
        if comp:
            ...
            info += f"Энтальпия: {comp.enthalpy:.2f} кДж/кг" if comp.enthalpy else "Энтальпия: N/A"
