...
class ResParser:
                ...
                if i + 1 < len(lines):
                    ...
                    if name_match:
                        ...
                        if re.search(r'[A-Z]', name, re.IGNORECASE):
                            self.data.components.append(ResComponent(name=name, hf298=hf298))
            i += 1
    ...
    def _parse_element_composition(self, content: str):
        ...
        for match in pattern.finditer(content):
            self.data.element_composition[match.group(1).strip()] = float(match.group(2))
    ...
    def _parse_all_calculations(self, content: str):
        ...
        for idx, marker_idx in enumerate(calc_markers):
            ...
            prev_idx = calc_markers[idx - 1] + 20 if idx > 0 else max(0, marker_idx - 15)
            next_idx = calc_markers[idx + 1] if idx + 1 < len(calc_markers) else len(lines)
            ...
            calc = self._parse_single_calculation(idx + 1, section_lines, all_lines, marker_offset)
            if calc:
                self.data.calculations.append(calc)

    def _parse_single_calculation(self, calc_id: int, section_lines: list, all_lines: list, marker_offset: int) -> CalculationResult:
        ...
        for i in range(marker_offset, len(section_lines)):
            ...
            if in_composition:
                ...
                match = re.match(r'\s*(?:\|\s*)?([A-Z][A-Za-z0-9]*)\*\s+([\d.E+-]+)', line)
                if match:
        ...
        # Дата и время
        date_match = re.search(r'(\d{2}:\d{2}:\d{2})\s+(\d{2}\.\d{2}\.\d{2})', '\n'.join(all_lines))
        if date_match:
...
def print_structured_data(data: ResData):
    ...
    if data.calculations:
        ...
        for calc in data.calculations:
            ...
            print(f"  Thermodynamic parameters:")
            if calc.pressure: print(f"    Pressure (P):        {calc.pressure:.6e}")
            if calc.temperature: print(f"    Temperature (T):     {calc.temperature:.2f} K")
            if calc.enthalpy: print(f"    Enthalpy (I):        {calc.enthalpy:.2f}")
            if calc.entropy: print(f"    Entropy (S):         {calc.entropy:.4f}")
            if calc.heat_capacity: print(f"    Heat capacity (C):   {calc.heat_capacity:.4f}")
            if calc.density: print(f"    Density (R):         {calc.density:.4f}")
            if calc.molar_mass: print(f"    Molar mass (M):      {calc.molar_mass:.2f}")
            if calc.adiabatic_index: print(f"    Adiabatic index (K): {calc.adiabatic_index:.4f}")
            if calc.volume_gas: print(f"    Volume gas (V гф):     {calc.volume_gas:.4f}")
            if calc.condensed_fraction: print(f"    Condensed frac (Z кф): {calc.condensed_fraction:.4f}")
            
            # Равновесный состав газовой фазы
            if calc.equilibrium_gas:
                print(f"  Equilibrium gas ({len(calc.equilibrium_gas)} components):")
                sorted_gas = sorted(calc.equilibrium_gas.items(), key=lambda x: -x[1])[:15]
